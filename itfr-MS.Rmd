...
# Main Text
Fertility is the primary engine of global population change [@gerland2014] and is linked to the United Nation's Sustainable Development Goals for female education, child and maternal mortality, gender equality, and reproductive health [@abel16]. The total fertility rate (TFR; the expected number of children born over a complete reproductive lifetime) is widely regarded as one of the most important components of population change, and demographers use it in a wide range of applications [@yule1906changes;@lutz2011;@shenk13;@lesthaeghe14;@myrskyla;@lee;@kaiser;@conley;@jones;@currie].
...
Here we discuss a modeling framework that overcomes these problems and demonstrate the near-universal applicability of a parameter-free estimation method. The method uses census or survey counts of population by age and sex, and its principles are straightforward and well known. Recent research has demonstrated that the basic method's errors are acceptably small (@@@CITATIONS HERE@@@), and that several modifications can make errors smaller still (@@@CITATIONS HERE@@@).

We first approximate TFR using a variant of the general fertility rate (GFR; the total observed births divided by the total number of reproductive age women), in which we substitute the number of enumerated children for recorded births. Then, based only on accurate population counts by age and sex, we develop a more complex version that is accurate over numerous scales, time periods, sub-populations, and even species. Finally, we combine simple age-sex distributions with commonly available estimates of child mortality to produce a Bayesian estimator of TFR that performs better still. 
